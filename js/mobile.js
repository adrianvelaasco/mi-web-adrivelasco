document.addEventListener("DOMContentLoaded", function () {
    ...
    function initParticles() {
        ...
        const layers = [
            { step: 6, alpha: 0.5, factor: 0 },    // ~50% of original density (original: 4)
            { step: 13, alpha: 0.15, factor: 0.2 }, // ~50% of original density (original: 10)
            { step: 21, alpha: 0.08, factor: 0.4 }  // ~50% of original density (original: 18)
        ];
        ...
        forYouView.innerHTML = displayWorks.map(work => `
            <div class="foryou-card">
                ...
                <div class="foryou-video-container">
                    <video data-src="${work.video}" loop ${isMuted ? 'muted' : ''} playsinline preload="metadata"></video>
                </div>
                <div class="foryou-content">
                    <h3 class="foryou-title">${work.title}</h3>
                </div>
                <a href="works/viewer_mobile.html?work=${work.id}" class="foryou-link" style="position:absolute; inset:0; z-index:5;"></a>
            </div>
        `).join('');
